...
private:
    ...
    void ClearHashConstants(){//just set them to default states
		hash_constants = {
			0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
		};
    }
...
public:
	...
	/* Overloaded hash so if called with a vector of bytes, can be used for that also. instead of pretty much rewriting most of PreProcessMessage, just make a string then do the already established flow of this class*/
	std::vector<uint8_t> hash(const std::vector<uint8_t>& input){
		std::string pre_hash_str (input.begin(), input.end());
        PreProcessMessage(pre_hash_str);
        ClearHashConstants();
        for (size_t i = 0; i < blocks.size(); i++){
            compression(blocks[i]);
        }//compression updates the hashed constants for each block, now all thats left is to slap em into the output hash
	...
	//to eyeball as a string or any other reason you would need the string represntation returned
    std::string hash_print(const std::string& input){ 
		PreProcessMessage(input);
